...
public class DamageHud extends AbstractHud {
    ...
    protected void configure(@Nonnull Configuration config) {
        this.noSelTimeConf = config.getInt("noSelectTime", getConfigCategory(), noSelTimeConf, 0, 1000, "The time to show the mob-hud whilst not hovering over it anymore. (In ticks, 1/20th of a second)");
        this.deathShowConf = config.getInt("timeShowDeath", getConfigCategory(), deathShowConf, 0, 1000, "The time to show the mob-hud of an entity whilst it's dead. (In ticks, 1/20th of a second)");
    }
